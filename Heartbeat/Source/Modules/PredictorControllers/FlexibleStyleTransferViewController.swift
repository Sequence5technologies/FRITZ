...
class FlexibleStyleTransferViewController: FeatureViewController {
  ...
  convenience init() {
    ...
    let group = ModelGroupManager(initialModel: details, tagName: "heartbeat-ios-flexible-style-transfer")
  ...
  override func build(_ predictorDetails: FritzModelDetails) -> HeartbeatImagePredictor? {
    ...
    if let predictor = try? FritzVisionFlexibleStyleModel(model: model) {
      return HeartbeatImagePredictor(model: predictor, predictorDetails: predictorDetails)
    }
    return nil
  }
}
