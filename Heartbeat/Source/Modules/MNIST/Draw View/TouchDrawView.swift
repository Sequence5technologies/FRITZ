...
public protocol TouchDrawViewDelegate: class {
    ...
    /// triggered when drawing has stopped
    func drawingStopped()
}
...
open class TouchDrawView: UIView {
    ...
    /// Time to wait before notifying delegate of draw end
    open var drawStopDelay: TimeInterval = 0.7
    ...
    /// Timer used to track when drawing has stopped
    fileprivate var drawTimer: Timer?
...
extension TouchDrawView {
    ...
    override open func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            ...
            drawTimer = Timer(timeInterval: drawStopDelay, repeats: false) { [weak self] _ in
                self?.delegate?.drawingStopped()
            }
            
            RunLoop.main.add(drawTimer!, forMode: .commonModes)
        }
    }
